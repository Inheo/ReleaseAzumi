...
public class ParticleGenerator : MonoBehaviour {		
	...
	public int COUNT_SPAWN_LIQUID_PARTICLE = 50;
	...
	public Pumice Pumice;

	void Start() {
		StartCoroutine("SpawnLiquidParticle");
		if(particlesState == DynamicParticle.STATES.WATER)
        {
			particlesParent = AllDataInScene.Instance.ParentWater.transform;
        }
        else
		{
			particlesParent = AllDataInScene.Instance.ParentLava.transform;
		}
	}
	...
	private IEnumerator SpawnLiquidParticle()
    {
		for (int i = 0; i < COUNT_SPAWN_LIQUID_PARTICLE; i++)
		{
			...
			particleScript.IsDisappearing = isDisappearing;
			if (Pumice != null)
			{
				Pumice.CountOfCollisionsToFill = (int)(COUNT_SPAWN_LIQUID_PARTICLE * 0.8f);
				particleScript.CollisionParticle += Pumice.AddFillPercent;
				Pumice.WhenFilling += particleScript.IsDisappearingTrue;
			}
